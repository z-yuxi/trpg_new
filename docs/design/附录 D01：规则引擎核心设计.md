...
| 参数 | 值 | 说明 |
|------|-----|------|
| 单次指令执行超时 | **500ms** | 超过后强制终止 isolated-vm 实例并返回 `ENGINE_TIMEOUT` |
| 单实例内存上限 | **64MB** | isolated-vm `memoryLimitMb` 参数，超出触发 OOM 终止 |
| 最大递归调用深度 | **16** | 防止配方链无限递归，超出返回 `ENGINE_RECURSION_LIMIT` |
...
| 错误码 | 触发条件 |
|--------|---------|
| `ENGINE_TIMEOUT` | 单次执行超过 500ms |
| `ENGINE_OOM` | 内存超过 64MB |
| `ENGINE_RECURSION_LIMIT` | 递归深度超过 16 |
| `ENGINE_LOOP_LIMIT` | 循环超过 1000 次 |
| `ENGINE_PARSE_ERROR` | DSL 语法错误 |
| `ENGINE_UNKNOWN_RECIPE` | recipe_id 不存在或未编译 |
...
借鉴 Function Calling 的 Schema 定义模式，每个规则包的自定义指令都附带一个明确的 JSON Schema，描述该指令的输入参数、输出格式和示例。引擎在加载规则包时，将所有指令的 Schema 注册到解析器中，用于：

1. **参数校验**：在调用执行器之前，自动校验用户输入是否匹配 Schema 定义。
2. **助理台渲染**：助理台可依据 Schema 自动生成指令速查面板（参数名、类型、是否必填、示例值）。
3. **错误提示**：当用户输入不符合 Schema 时，提供结构化的错误信息，而非笼统的「指令解析失败」。

##### Schema 结构

每个自定义指令的 Schema 包含以下字段：

| 字段 | 类型 | 说明 |
|------|------|------|
| `trigger` | string | 触发词或正则 |
| `description` | string | 人类可读的指令描述 |
| `parameters` | array | 参数列表（每个参数含 name / type / required / description / example） |
| `output_format` | string | 输出文本模板（如 `{skill_name}检定：{roll}/{target} → {result}`） |
| `examples` | array | 使用示例（含输入和预期输出） |
...
```json
{
  ...
  "output_format": "{skill_name}检定：{roll}/{target} → {result}",
  "examples": [
    {
      "input": "ra 侦查",
      "output": "侦查检定：35/50 → 成功"
    }
  ]
}
```
...
1. 规则包加载时，引擎遍历所有指令的 Schema，注册到解析器中。
2. 用户输入指令时，解析器匹配 `trigger` → 提取参数 → 按 Schema 校验必填项和类型。
3. 校验通过 → 调用执行器；校验失败 → 返回结构化错误（缺失参数 / 类型不符 / 未知参数）。
4. 校验结果和 Schema 中的 `output_format` 一并传递给执行器，用于生成展示文本。

##### 验收标准

- 规则包作者在 Recipe 编辑器中定义自定义指令时，可预览 Schema 校验效果
- 助理台指令速查基于 Schema 自动生成，不需要前端硬编码
- 指令解析错误时，用户能看到具体哪个参数有问题
  li:                         # 总结疯狂
    component: "random_table"
