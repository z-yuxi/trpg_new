...
本附录遵循 `docs/文档对齐治理.md`，本文只负责 ID 规范，不对上游准则做反向覆盖。
...
- **UID**：统一为 **7 位数字**。
  - 普通用户：从 `1000100` 开始顺序递增。用于个人主页（`/u/1000103`）、@提及、对外展示。
  - **系统保留号段**：`1000000` - `1000099` 共 100 个，分配给平台管理员、内部运营及测试账号使用。该号段不纳入普通用户的自增序列，也不通过前台注册接口自动分配。
