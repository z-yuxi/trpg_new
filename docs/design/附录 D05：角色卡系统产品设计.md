...
> 当创作者发布规则包新版本（`ruleset_version` 递增），已使用该规则包的角色卡如何处理新增/变更字段，以下为 V1.0 的约束口径。

**核心原则：新字段自动填充默认值，旧字段保留原始值，不强制更新。**

| 变更类型 | 处理策略 | 说明 |
|---------|---------|------|
| 规则包新增字段 | **自动填充字段默认值**（`null` / `0` / `""`，取字段 schema 中的 `default`） | 角色卡不中断，团内无感知 |
| 规则包删除字段 | **保留旧数据**，前端渲染时忽略未知字段 | 不删除已有数据，防止数据丢失 |
| 规则包修改字段类型/枚举值 | **不自动迁移**，运维脚本手动处理（需创作者提供迁移说明） | 类型变更为破坏性变更，需创作者自行管理版本 |
| 规则包修改字段默认值 | **不影响已有角色卡**，仅对新建角色卡生效 | 避免已有数据被静默修改 |

**触发时机**：GM 在团内手动点击"同步规则包版本"，或 GM 进入房间时系统检测到版本差距，提示更新（不强制）。  
**不自动同步**：规则包发布不触发所有已用角色卡的后台批量更新，避免大规模数据写入风险。

**字段版本记录**：`character_templates.ruleset_version` 存储该卡创建时绑定的规则包版本号，供前端判断是否需要提示升级。
