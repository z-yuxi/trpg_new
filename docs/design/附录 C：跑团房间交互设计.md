...
**页面结构**：一级Tab：`找房间` / `开团招人` / `我的`（移动端横向滚动，桌面端正常展示）
...
- 模组选择器：从 GM 的已获取资产（「我的」→资产）中查询已拥有的模组。
...
- **可选**：模组（下拉选择："无模组" / 从已获取资产（「我的」→资产）中选择已有模组）。
...
| 在创作者后台/讨论/招募/探索/我的页面 | 显示全局底部导航，隐藏房间底部快捷栏 |
