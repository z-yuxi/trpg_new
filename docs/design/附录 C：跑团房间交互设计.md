...
**页面结构**：一级Tab：`找团玩` / `当GM` / `我的`（移动端横向滚动，桌面端正常展示）
...
- **空状态**：同时显示两个按钮：`[去当GM看看]` + `[发布第一个招募帖]`。

##### 4.4.1.1 交互状态细则：招募帖卡片操作按钮

> 本表覆盖"找团玩"Tab 的 GM 招募帖卡片和"当GM"Tab 的玩家求组帖卡片的所有操作按钮状态。

###### 找团玩（GM招募帖）卡片按钮

| 状态 | 触发条件 | 按钮文案 | 是否禁用 | 额外视觉 | 点击行为 | 失败兜底 |
|:---|:---|:---|:---|:---|:---|:---|
| `can_apply` | 已登录，非发帖GM，招募帖 `open`，席位未满 | 申请加入 | 否 | - | 弹出角色卡选择弹窗 | Toast "操作失败，请重试" |
| `pending` | 已提交申请，GM 未审核 | 已申请 | 是（灰底） | 时钟图标 | 无 | - |
| `invited` | GM 已通过，等待确认 | 确认入团 (N小时后失效) | 否（蓝底强调） | 倒计时文案实时刷新 | 提交确认请求 | 网络失败进入重试队列 |
| `confirmed` | 已确认入团 | 已加入 | 是（灰底+绿色对勾） | - | 无 | - |
| `waiting` | 满员后进入候补 | 候补中 (第N位) | 是（灰底） | - | 无 | - |
| `full_no_ob` | 已成团，不允许观战 | 已满员 | 是（灰底） | - | 无 | - |
| `can_spectate` | 已成团，`allow_spectate = true`，当前用户非团员 | 观看直播 | 否（绿底） | 👁️ 图标 | 进入房间观战模式（只读） | 房间不存在时提示"房间已关闭" |
| `closed` | 招募帖 `closed` / `grouped` | 已结束 | 是（灰底） | - | 无 | - |
| `not_login` | 未登录 | 申请加入 | 否，点击时拦截 | - | 弹出登录/注册模态框 | - |
...
> **移动端适配**：移动端的筛选交互见《附录 B：社区产品设计规范》移动端适配节。
...
**交互规则**：
- GM 在"当GM"列表的求组帖卡片上点击 `[邀请入团]` 时触发此模态框。
