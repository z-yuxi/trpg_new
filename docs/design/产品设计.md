...
- 《[附录 C01：跑团房间交互设计](附录 C01：跑团房间交互设计.md)》
- 《[附录 D01：规则引擎核心设计](附录 D01：规则引擎核心设计.md)》
- 《[附录 E01：模组编辑器产品设计](附录 E01：模组编辑器产品设计.md)》
...
- 通用指令（平台内置）：`['r', 'rh', 'nn']`，始终显示。    
- 其余如 `jrrp`、`draw`、`log`、`ob` 若后续恢复为平台内置通用指令，应先补服务端默认命令定义，再更新本节口径。    
...
《[附录 M：会员权限与商业化口径](附录 M：会员权限与商业化口径.md)》。
...
| 8   | 模组详情直达招募入口 | 用户在模组详情页可直接查看该模组当前招募帖，缩短“看模组 -> 找团/开团”路径 | ① 模组详情页展示“正在招募（N）”入口；② 点击后进入招募板并自动带入模组筛选；③ 支持查看该模组下 open/full/grouped 状态分布 | 招募帖与模组建立 module_id 关联；列表接口支持 module_id 精确过滤 |
...
- 模组详情直达招募：模组详情页入口跳转到招募板并携带 `module_id` 过滤参数；招募列表优先使用 `module_id` 精确匹配，`module_name` 仅用于历史数据兼容展示。
