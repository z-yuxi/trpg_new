...
| 文档版本 | 日期         | 说明                                                |
| ---- | ---------- | ------------------------------------------------- |
| v1.0 | 2026-04-08 | 轻量节点图架构，移除工业级 FC/FB/BDB 概念，基于原 Excel/附录 F/附录 C 整合 |
...
- **低门槛**：规则作者通过 YAML 配置或可视化节点图定义规则，无需编写代码。
...
| P-002 | 配置即行为 | 所有规则行为通过规则包 YAML 定义 |
...
### 2.2 规则包（Ruleset）
- **定义**：一个 YAML 文件，包含 `atoms`（节点实例）、`connections`（连线）、`commands`（指令映射）。
- **存储格式**：
```yaml
meta:
  id: "coc7_skill"
...
interface ExecuteRequest {
  ruleset_id: string;
  command: string;               // 如 "ra"
  params: Record<string, any>;   // 如 { skill: "侦查" }
  context: {
    character_id: string;
    campaign_id: string;
    scene_id?: string;
...
commands:
  - trigger: "ra {skill}"
    entry_atom: "get_skill"       # 先从角色卡读取技能值
    output_atom: "output"
    input_mapping:
      skill: "{skill}"
...
（本节直接引用《附录 C：技术约定》第 10 章，内容保持不变）
...
（仅列出与引擎直接相关的表，完整设计见《附录 C：技术约定》第 12 章）
...
> **基于附录 F v1.0 的扩展**，解决指令系统、派生值、职业解析器、动作调度器、事务处理、热加载等关键缺失。  
> **目标**：使引擎完整支撑产品设计 V2.0 中规则工坊、角色卡系统、回合战术的全部需求。
...
```typescript
// 规则包中的指令声明（扩展附录 F 的 commands 字段）
...
根据最终配置的 component 字段找到入口原子 ID
       ↓
按照 input_mapping 将用户参数填入原子输入端口
       ↓
执行节点图，返回结果
```
...
- 所有职业数据存储在 `occupation_definitions` 表（附录 E 已有结构，需补充 `progression_table` 和 `feature_graph` 字段）。
