...
export async function up(knex: Knex): Promise<void> {
  ...
  if (!(await knex.schema.hasTable('post_replies'))) {
    await knex.schema.createTable('post_replies', (t) => {
  ...
  if (!(await knex.schema.hasTable('reply_comments'))) {
    await knex.schema.createTable('reply_comments', (t) => {
      t.string('id', 64).primary();
  ...
  // 楼层点赞防重复
  if (!(await knex.schema.hasTable('floor_likes'))) {
    await knex.schema.createTable('floor_likes', (t) => {
      t.string('id', 64).primary();
