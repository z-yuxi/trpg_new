...
import type { CharacterCardSchema } from '../../components/rule-canvas/CharacterCardSchemaEditor.vue';
import RecruitmentFieldsEditor from '../../components/RecruitmentFieldsEditor.vue';
import type { RecruitmentField } from '../../components/RecruitmentFieldsEditor.vue';
import CommandOverridesEditor from '../../components/CommandOverridesEditor.vue';
import type { CommandOverride } from '../../components/CommandOverridesEditor.vue';
...
import { validateRulesetForm } from '../../utils/ruleset-validator';
...
type InputMappingParamType = 'string' | 'number' | 'boolean';
type CustomInputMapping = { param_name: string; source: InputMappingSource; fixed_value?: string; field_name?: string; param_type?: InputMappingParamType; description?: string };
type CustomCommand = {
  trigger: string;
  description: string;
  aliases: string;        // 逗号分隔
...
async function save() {
  // 校验 L1 表单字段
  const validationErrors = validateRulesetForm({
    name: formName.value,
    defaultDice: defaultDice.value,
    checkMode: checkMode.value,
    critSuccessMax: critSuccessMax.value,
    critFailMin: critFailMin.value,
    difficultyLevels: difficultyLevels.value,
    attributes: attributes.value,
    resources: resources.value,
    supportedCommands: supportedCommands.value,
    customCommands: customCommands.value.map((c) => ({
      name: c.trigger,
      param_mapping: (c.input_mapping ?? []).map((m: any) => ({ source: m.source, field: m.field })),
    })),
  });
  ...
  if (validationErrors.length > 0) {
    ...
    // 滚动到对应字段
    const el = document.querySelector(`[data-field="${first.field}"]`) as HTMLElement | null;
  ...
  const serializedCustomCommands = customCommands.value.map((cmd) => {
    ...
    try { graph = JSON.parse(cmd.graph_json); } catch (e) {
      console.error(`自定义命令 '${cmd.trigger}' 的 graph_json 解析失败:`, e);
      ElMessage.warning(`自定义命令「${cmd.trigger}」的画布数据损坏，已跳过`);
    }
    return {
      trigger: cmd.trigger,
              ...
              <table v-else class="mapping-table">
                ...
                <tbody>
                  <tr v-for="(mp, mi) in cmd.input_mapping" :key="mi">
                    <td>
                      <input
                        v-model="mp.param_name"
                        class="field-input mapping-input"
                        placeholder="如 skill"
                        @blur="() => { if (mp.param_name && !/^[a-z][a-z0-9_]*$/.test(mp.param_name)) { $el?.closest('td')?.querySelector('input')?.classList.add('input-error'); } }"
                      />
                    </td>
                    <td>
                      <select v-model="mp.param_type" class="status-select mapping-select" style="width: 80px;">
                    ...
                    <td>
                      ...
                      <span v-if="mp.source === 'user_input'" class="hint-text" style="font-size: 11px;">由用户输入</span>
                    </td>
                    <td>
                      <input v-model="mp.description" class="field-input mapping-input" placeholder="参数说明（可选）" />
                    </td>
