...
<template>
  ...
  <teleport to="body">
    ...
    <div v-if="menuVisible" class="ctx-menu" :style="{ left: menuPos.x + 'px', top: menuPos.y + 'px' }">
      ...
      <button class="ctx-menu__item" @click="copyTimestamp">📝 复制时间</button>
      <button v-if="message._sendStatus === 'failed'" class="ctx-menu__item" @click="emit('retry', message._tempId); closeMenu()">🔄 重新发送</button>
      <button v-if="!isOwn" class="ctx-menu__item ctx-menu__item--danger" @click="openReport">⚠️ 举报</button>
    </div>
