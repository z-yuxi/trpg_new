...
const props = defineProps<{ modelValue: JsonArray; supportedCommands?: string[] }>();
...
<template>
  <div class="json-editor-wrapper">
    <p v-if="(supportedCommands?.length ?? 0) > 0" class="hint">
      可覆盖指令：{{ supportedCommands?.join(', ') }}
    </p>
    <textarea
      v-model="draft"
      class="json-editor"
