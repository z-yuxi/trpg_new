...
exports.mergeRulesetGraphs = mergeRulesetGraphs;
/**
 * 三方合并
 ...
 */
function mergeRulesetGraphs(base, ours, theirs) {
    ...
    for (const [id, theirNode] of theirMap) {
        ...
        if (!ourNode) {
            if (baseNode) {
                ...
                if (theirChanged) {
                    // theirs 也修改了，冲突
                    conflicts.push({ node_id: id, type: 'modified_both', our_node: null, their_node: theirNode });
                }
    ...
    for (const [id, ourNode] of ourMap) {
        ...
        // 若我们也未改，接受删除；若我们改了，冲突
            ...
            if (ourChanged) {
                conflicts.push({ node_id: id, type: 'modified_both', our_node: ourNode, their_node: null });
                merged.push(ourNode);
            }
            // else: 我们未改，接受删除
        }
...
//# sourceMappingURL=ruleset-merger.js.map