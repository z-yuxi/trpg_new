...
import { mergeRulesetGraphs } from '../ruleset-merger';
...
describe('mergeRulesetGraphs', () => {
  ...
  it('theirs 新增节点 — 自动合并', () => {
    ...
    const result = mergeRulesetGraphs(base, ours, theirs);
    const ids = result.merged_graph.atoms.map((n) => n.node_id);
  ...
  it('只有 theirs 修改节点 — 接受 theirs', () => {
    ...
    const result = mergeRulesetGraphs(base, ours, theirs);
    expect(result.merged_graph.atoms[0].label).toBe('updated by parent');
    expect(result.conflicts).toHaveLength(0);
  });
  ...
  it('只有 ours 修改节点 — 保留 ours', () => {
    ...
    const result = mergeRulesetGraphs(base, ours, theirs);
    expect(result.merged_graph.atoms[0].label).toBe('local edit');
    expect(result.conflicts).toHaveLength(0);
  });
  ...
  it('双方都修改同一节点且内容不同 — 产生冲突，保留 ours', () => {
    ...
    // ours 保留
    expect(result.merged_graph.atoms[0].label).toBe('mine');
  });
  ...
  it('双方修改同一节点但结果相同 — 无冲突', () => {
    ...
    const result = mergeRulesetGraphs(base, ours, theirs);
    expect(result.conflicts).toHaveLength(0);
    expect(result.merged_graph.atoms[0].label).toBe('same');
  });

  it('theirs 删除节点，ours 未修改 — 接受删除', () => {
    const base = { atoms: [node('A'), node('B')], connections: [] };
    const ours = { atoms: [node('A'), node('B')], connections: [] };
    const theirs = { atoms: [node('A')], connections: [] };  // B deleted
    const result = mergeRulesetGraphs(base, ours, theirs);
    const ids = result.merged_graph.atoms.map((n) => n.node_id);
